...
class GatedGroup(nn.Module):
    def __init__(self, res_blocks, dim, down_sample, expansion_ratio):
        super().__init__()
        self.register_buffer(
            "down_sample", torch.tensor(down_sample, dtype=torch.uint8)
        )
        self.body = nn.Sequential(
            *[
                GatedCNNBlock(dim, down=down_sample, expansion_ratio=expansion_ratio)
                for _ in range(res_blocks)
            ]
            + [HybridAttention(dim, down_sample)]
        )
...
class RHA(nn.Module):
    ...
    def __init__(
        ...
        if scale < 4 and unshuffle_mod:
            ...
            unshuffle = 4 // scale
            self.register_buffer(
                "unshuffle", torch.tensor(unshuffle, dtype=torch.uint8)
            )
            scale = 4
        ...
        self.body = nn.Sequential(
            *[
                GatedGroup(
                    res_blocks, dim, down_list[i % down_sample_len], expansion_ratio
                )
